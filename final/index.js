var I=new Set,l=({id:_,cssText:z})=>{if(I.has(_)||document.querySelector(`#${_}`))return;const K=document.createElement("style");K.id=_,K.textContent=z,K.classList.add("evenbetterapi-css-module"),document.head.appendChild(K),I.add(_)},Y=l;var i=`
.eb-checkbox__input {
    cursor: pointer;
...
`;document.addEventListener("keydown",(_)=>{if(_.key==="Escape")E()});var n=({title:_,content:z})=>{Y({id:"evenbetterapi-modal",cssText:o.toString()});const K=document.createElement("div");K.classList.add("evenbetter-modal");const F=document.createElement("div");F.classList.add("evenbetter-modal__content");const Z=document.createElement("div");Z.classList.add("evenbetter-modal__content-header");const q=document.createElement("h2");q.classList.add("evenbetter-modal__content-header-title"),q.textContent=_,Z.appendChild(q);const G=document.createElement("div");G.classList.add("evenbetter-modal__content-body");const J=document.createElement("p");J.classList.add("evenbetter-modal__content-body-text"),J.textContent=z;const Q=document.createElement("button");return Q.classList.add("evenbetter-modal__content-body-close"),Q.textContent="Close",Q.addEventListener("click",E),G.appendChild(J),G.appendChild(Q),F.appendChild(Z),F.appendChild(G),K.appendChild(F),K.setAttribute("data-modal-title",_),K},t=()=>{return document.querySelector(".evenbetter-modal")!==null},E=()=>{document.querySelector(".evenbetter-modal")?.remove()},S=({title:_,content:z})=>{if(t())E();const K=n({title:_,content:z});document.body.appendChild(K)};var e=`
.v-toast--fade-in {
...
}`,_1=(_,z,K)=>{const F=document.createElement("div");F.classList.add("v-toast"),F.classList.add(`v-toast--${K}`);const Z=document.createElement("div");Z.setAttribute("role","alert"),Z.classList.add("v-toast__item"),Z.classList.add(`v-toast__item--${z}`),Z.classList.add(`v-toast__item--${K}`),F.appendChild(Z);const q=document.createElement("div");q.classList.add("v-toast__icon"),Z.appendChild(q);const G=document.createElement("p");return G.classList.add("v-toast__text"),G.textContent=_,Z.appendChild(G),F.classList.add("v-toast--fade-in"),F},M=(_)=>{const{message:z,type:K,position:F,duration:Z}=_;Y({id:"eb-toast",cssText:e});let q=document.querySelector(`.v-toast--${F}`);if(!q)q=document.createElement("div"),q.classList.add("v-toast"),q.classList.add(`v-toast--${F}`),document.body.appendChild(q);const G=_1(z,K,F);q.appendChild(G),setTimeout(()=>{G.classList.add("v-toast--fade-out"),setTimeout(()=>{G.remove()},150)},Z-150)};function C(_,z="Search...",K=!1,F){Y({id:"eb-text-input",cssText:F1});const Z=document.createElement("div");Z.classList.add("formkit-outer","c-text-input__outer"),Z.setAttribute("style",`width: ${_};`);const q=document.createElement("div");q.classList.add("formkit-wrapper"),q.style.display="flex";const G=document.createElement("div");G.classList.add("formkit-inner","eb-text-input__inner");const J=document.createElement("div");J.classList.add("c-text-input__prefix"),J.setAttribute("style","align-self: center; color: var(--c-fg-subtle);");const Q=document.createElement("i");if(Q.classList.add("c-icon","fas"),F)Q.classList.add(F);const X=document.createElement("input");if(X.setAttribute("placeholder",z),X.setAttribute("spellcheck","false"),X.classList.add("formkit-input","c-text-input__input"),X.setAttribute("type","text"),X.setAttribute("style","width: 100%; background: transparent; border: 0; outline: 0; color: inherit; box-sizing: border-box; line-height: inherit;"),F)J.appendChild(Q);if(G.appendChild(J),G.appendChild(X),K){const V=document.createElement("button");V.innerHTML='<i class="fas fa-copy"></i>',V.setAttribute("style","background: transparent; border: 0; outline: 0; cursor: pointer; padding: 0; margin-left: 10px;"),V.addEventListener("click",()=>{navigator.clipboard.writeText(X.value),U.toast.showToast({message:"Copied to clipboard",position:"bottom",type:"info",duration:1000})}),G.appendChild(V)}return q.appendChild(G),Z.appendChild(q),Z}var F1=`
...
`;class D{handlers=[];commandObserver=null;selectedCommand=null;addHandler(_){this.handlers.push(_)}init(){const _=(F)=>{this.commandObserver=new MutationObserver((Z)=>{const q=z();if(q!==this.selectedCommand)this.selectedCommand=q}),this.commandObserver.observe(F,{attributes:!0,subtree:!0})},z=()=>{return document.querySelector("[command-item][aria-selected='true']")};new MutationObserver((F)=>{for(let Z of F)if(Z.type==="childList")Z.addedNodes.forEach((q)=>{const G=q;if(q.nodeType===1&&G.hasAttribute("command-theme"))_(G)}),Z.removedNodes.forEach((q)=>{const G=q;if(q.nodeType===1&&G.hasAttribute("command-theme")){if(!this.selectedCommand)return;const J=this.selectedCommand.getAttribute("data-value");if(J)this.trigger(J);if(this.commandObserver)this.commandObserver.disconnect(),this.commandObserver=null}})}).observe(document.body,{childList:!0,subtree:!0})}trigger(_){this.handlers.forEach((z)=>z(_))}}var $=window.Caido;var g=[],v=(_)=>{_.on("onCommandRun",(z)=>{const K=g.find((F)=>F.commandName===z);if(!K)return;z1(K)})},z1=(_)=>{const z=document.createElement("div");z.classList.add("custom");const K=document.createElement("div");K.setAttribute("command-root",""),z.appendChild(K);const F=document.createElement("div");F.setAttribute("command-dialog",""),K.appendChild(F);const Z=document.createElement("div");Z.setAttribute("command-dialog-mask",""),Z.style.display="flex",Z.style.justifyContent="center",Z.style.alignItems="center",Z.addEventListener("click",()=>{z.remove()}),F.appendChild(Z);const q=document.createElement("div");q.setAttribute("command-dialog-wrapper",""),q.style.minWidth="600px",q.style.padding="1em",q.style.margin="0",Z.appendChild(q);const G=document.createElement("div");G.setAttribute("command-dialog-body",""),G.style.display="flex",G.style.gap="0.5em",q.addEventListener("click",(X)=>{X.stopPropagation()}),q.appendChild(G);const J=U.components.createTextInput("100%",_.promptPlaceholder);J.style.zIndex="100",J.addEventListener("click",(X)=>{X.stopPropagation()}),G.appendChild(J);const Q=$.ui.button({label:"Submit",variant:"primary"});Q.addEventListener("click",()=>{const X=J.querySelector("input");_.onPrompt(X.value),z.remove()}),G.appendChild(Q),document.body.appendChild(z),J.querySelector("input")?.focus(),J.addEventListener("keydown",(X)=>{if(X.key==="Enter")Q.click();if(X.key==="Escape")z.remove()})},K1=(_,z,K)=>{g.push({commandName:_,promptPlaceholder:z,onPrompt:K})},c={createPromptCommand:K1};var u=async()=>{if(localStorage.getItem("previousPath"))setTimeout(()=>{window.location.hash=localStorage.getItem("previousPath")||"",localStorage.removeItem("previousPath")},200);const _=new WebSocket("ws://localhost:8081");_.addEventListener("open",()=>{U.toast.showToast({message:"Connected to Caido Hot Reloading Server",duration:2000,type:"success",position:"bottom"})}),_.addEventListener("close",()=>{U.toast.showToast({message:"Disconnected from Caido Hot Reloading Server",duration:2000,type:"error",position:"bottom"})}),_.addEventListener("message",(F)=>{try{const Z=JSON.parse(F.data),q=Z.event,G=Z.data;switch(q){case"caido:loadJS":z(G,"");break;case"caido:loadCSS":z("",G);break;case"caido:reload":localStorage.setItem("previousPath",window.location.hash),location.reload();break;default:console.error("Unknown event type received:",q)}}catch(Z){console.error("Error parsing WebSocket message:",Z)}});const z=async(F,Z)=>{const q=await K();if(!q)return;const G={input:{data:q.data,migrations:q.migrations}};if(F)G.input.data.js=F;if(Z)G.input.data.css=Z;return $.graphql.updateViewerSettings(G)},K=async()=>{return await $.graphql.userSettings().then((F)=>{return F.viewer.settings})}};var Z1=`
  ...
  `},s=()=>{Y({id:"evenbetter-settings",cssText:p.toString()});const _=document.createElement("div");return _.innerHTML=Q1(),_.querySelectorAll("input[type=checkbox]").forEach((F)=>{F.addEventListener("change",(Z)=>{const q=Z.target;R(q.name,q.checked?"true":"false")})}),_.querySelector("#petSaveButton").addEventListener("click",()=>{const F=_.querySelector("#gif_url").value,Z=_.querySelector("#gif_width").value,q=_.querySelector("#gif_height").value;R("gif_url",F),R("gif_width",Z),R("gif_height",q)}),_};var r=()=>{$.navigation.addPage("/settings/theprimatepack",{body:s()}),$.menu.registerItem({type:"Settings",label:"The Primate Pack",path:"/settings/theprimatepack",leadingIcon:"fas fa-cat"}),$.commands.register("evenbetter:settings",{name:"Go to The Primate Pack: Settings",group:"The Primate Pack: Navigation",run:()=>{$.navigation.goTo("/settings/theprimatepack")}}),$.commandPalette.register("theprimatepack:settings");let _="<img src=\""+N("gif_url")+"\" width="+N("gif_width")+" height="+N("gif_height")+">";$.sidebar.registerItem(_,"/theprimatepack/activity",{icon:"",group:"The Primate Pack"}),$.sidebar.registerItem("Activity","/theprimatepack/activity",{icon:"fas fa-clock",group:"The Primate Pack"})};var Y1=()=>{r(),U.eventManager.on("onPageOpen",()=>{localStorage.setItem("previousPath",window.location.hash);const _=document.querySelector(".c-sidebar-item[data-is-active='true']");if(_){let z=_.querySelector(".c-sidebar-item__count");if(z)z.innerHTML=""}})};Y1();
